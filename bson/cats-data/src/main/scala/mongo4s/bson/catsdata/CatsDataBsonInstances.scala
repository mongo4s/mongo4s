...
trait CatsDataBsonInstances:
  ...
  given [A](using dec: BsonDecoder[Map[String, A]]): BsonDecoder[NonEmptyMap[String, A]] =
    dec.emap { map =>
      NonEmptyMap
        ...
        .toRight(BsonError.Custom("Expected a non-empty map"))
    }
  ...
  given [A, B](using encA: BsonEncoder[A], encB: BsonEncoder[B], tagA: ClassTag[A], tagB: ClassTag[B]): BsonEncoder[Ior[A, B]] =
    val (nameA, nameB, bothTag) = iorNames[A, B]
    {
      ...
      case Ior.Both(a, b) =>
        BsonDocument(IorDiscriminatorField, BsonString(bothTag))
          .append("left", encA.encode(a))
          .append("right", encB.encode(b))
    }
  end given
  ...
  given [A, B](using decA: BsonDecoder[A], decB: BsonDecoder[B], tagA: ClassTag[A], tagB: ClassTag[B]): BsonDecoder[Ior[A, B]] =
    ...
    def field(doc: BsonDocument, key: String): Either[BsonError, BsonValue] =
      ...
      then Right(doc.get(key))
      else Left(BsonError.MissingField(key))
    ...
    def discriminator(doc: BsonDocument): Either[BsonError, String] =
      field(doc, IorDiscriminatorField).flatMap { value =>
        ...
        then Right(value.asString.getValue)
          ...
          Left(
            BsonError.Nested(
              IorDiscriminatorField,
              BsonError.typeMismatch(BsonTypeName.String, value)
            )
          )
      }
