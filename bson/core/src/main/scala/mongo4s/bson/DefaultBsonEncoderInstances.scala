...
trait DefaultBsonEncoderInstances:
  ...
  given [A] => (encoder: BsonEncoder[A]) => BsonEncoder[Option[A]] =
    value => value.fold(BsonNull.VALUE)(encoder.encode)

  given [A] => (encoder: BsonEncoder[A]) => BsonEncoder[List[A]] =
    values => BsonArray(values.map(encoder.encode).asJava)

  given [A] => (encoder: BsonEncoder[A]) => BsonEncoder[Vector[A]] =
    values => BsonArray(values.map(encoder.encode).asJava)

  given [A] => (encoder: BsonEncoder[A]) => BsonEncoder[Seq[A]] =
    values => BsonArray(values.map(encoder.encode).asJava)

  given [A] => (encoder: BsonEncoder[A]) => BsonEncoder[Set[A]] =
    values => BsonArray(values.toList.map(encoder.encode).asJava)

  given [A] => (encoder: BsonEncoder[A]) => BsonEncoder[Map[String, A]] = values =>
