...
trait MongoDatabase[F[*], S[*]]:
  ...
  def getDirectCollection[A](collectionName: String)(using WireCodec[A], ClassTag[A]): F[MongoCollection[F, S, A]]
  ...
  def watch(options: WatchOptions[BsonDocument] = WatchOptions.default[BsonDocument])(using
      ...
      Streamable[S, ChangeEvent[BsonDocument]],
  ): S[ChangeEvent[BsonDocument]]
