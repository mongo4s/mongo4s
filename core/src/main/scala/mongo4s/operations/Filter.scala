...
enum Filter[E]:
  ...
  def toBson(naming: FieldNaming): BsonDocument = this match
    ...
    case Near(path, geometry, min, max, spherical) =>
      ...
      Filter.operator(naming, path, if spherical then "$nearSphere" else "$near", near)

    case GeoWithin(path, shape)        => Filter.operator(naming, path, "$geoWithin", shape.toBson)
    case GeoIntersects(path, geometry) => Filter.operator(naming, path, "$geoIntersects", BsonDocument("$geometry", geometry.toBson))
